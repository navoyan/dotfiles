...
local function pbar_draw()
    ...
    if state.pbar == PBAR_ACTIVE and duration then
        ...
        if vop or not state.thumbfast.disabled then
            ...
            local preview_req = { x = x, y = y, w = tw, h = th }
            mp.set_property_native("user-data/osc/hover-sec", hover_sec)
...
local function pbar_pressed()
    ...
    if state.duration then
        mp.set_property("time-pos", hover_to_sec(state.mouse.x, state.dpy_w, state.duration))
    end
end
...
local function pbar_update(next_state)
    ...
    if dpy_w == 0 or dpy_h == 0 or state.pbar == next_state or state.pbar == PBAR_UNINIT then
        return
    end
